/* 
    ...
    - Créez les variables nécessaires au projet, prenez 5 minutes pour réfléchir à un algorithme, cela vous donneras une idée de vos besoins.
...
let a=Math.floor(Math.random()*arrayOfQuotes.length);
let b=0;
console.log(arrayOfQuotes.length);
p[0].textContent = `${arrayOfQuotes[a][0]}`;
p[1].textContent = `${arrayOfQuotes[a][1]}`;
bouton.addEventListener("click", function(){
    while(a==b){
        a= Math.floor(Math.random()*arrayOfQuotes.length);
        console.log(a);
    };
    b=a;
    p[0].textContent = `${arrayOfQuotes[a][0]}`;
